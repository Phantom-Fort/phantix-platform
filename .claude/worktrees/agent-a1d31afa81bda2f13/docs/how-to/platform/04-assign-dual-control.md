# Platform: Assign audit-control (initiator & authorizer)

**Where:** **People & Control** → audit-control section  
**Why:** Sensitive writes (scans, DB changes, many mutations) require a short-lived operate session from initiator or authorizer.

![Users / audit control](../../screenshots/platform/users.png)
...
flowchart TD
  ...
  B & C --> D[Save audit-control configuration]
  D --> E[Later: actions recorded to the audit trail under the audit controller]
```
...
3. In audit-control settings:
   - Set **Initiator** to one user  
   - Set **Authorizer** to a **different** user  
4. Save.
5. Confirm the dashboard checklist shows audit-control configured.
...
- Reads usually do **not** need audit-control; writes often do.
